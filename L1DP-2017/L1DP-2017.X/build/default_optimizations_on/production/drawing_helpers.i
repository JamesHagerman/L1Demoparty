...
# 6 "drawing_helpers.c" 2

# 1 "system.h" 1
# 8 "drawing_helpers.c" 2
...
void rcc_color(uint16_t color) {
 ...
 G1CMDH = 0x6600;
    __builtin_nop();
    while(G1STATbits.RCCBUSY) continue;
}

void rcc_setdest(__eds__ uint8_t *buf) {
 while(!G1STATbits.CMDMPT) continue;
...
void rcc_w1tow2(__eds__ uint8_t *dest, __eds__ uint8_t *src) {
    ...
    while(G1STATbits.CMDFUL) continue;
    G1CMDL = (((int)80UL -1)<<12) | (int)480UL;
    G1CMDH = 0x6400 | (((int)80UL -1)>>8);
...
void lineFloat(float x1, float y1, float x2, float y2) {
    ...
    double hl=fabsf(x2-x1), vl=fabsf(y2-y1), length=(hl>vl)?hl:vl;
...
void cleanup(void) {
 ...
 rcc_color(0x00);
 rcc_draw(80UL -1,0, 1,480UL);
}

void drawBorder(uint16_t c) {

 rcc_color(c);
 rcc_draw(0,0, 1, 480UL);
 rcc_draw(80UL -2,0,1,480UL);
 rcc_draw(0,0, 80UL -2, PIX_H);
 rcc_draw(0,480UL -PIX_H,80UL -2,PIX_H);
}
...
void verBlind(void) {
 ...
 rcc_color(0);
 rcc_draw(0, 0, 80UL -1, blind);
 rcc_draw(0, 480UL -blind, 80UL -1, blind);
 if ( blind < 100) {
  blind+=2;
 }
}
