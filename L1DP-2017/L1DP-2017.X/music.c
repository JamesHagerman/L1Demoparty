...
//    }
    ...
    sample = sinetable2048[idx];
    // Spit the mixed value to the audio port:
    PORTB=(sample<<8); // shift is to get to the right pins

    // The Sample clock is running at 44.1kHz (or close enough). To pull waves
    // from the wavetable, we need to follow this equation:
    //  S = N * (f/Fs)
    //  S = step size, N = size of wavetable, Fs is stample rate, f is note freq

    // TODO: Grab freq from song array per channel
    // Note: Keep in mind, we're in integer land here!!! Gotta scale everything!

    uint32_t freq = noteFreq; // start with A4 for giggles
    const uint32_t wtSize = 2048UL; // Not all of them are. Be sure of this
    const uint32_t sampleRate = 441UL;
    uint32_t stepSize = (wtSize*(freq/sampleRate))/10000UL;

    idx += stepSize;
    if (idx >= 2048) { // use sizeof()
        idx = idx - 2048;
    }
...
const unsigned char sinetable2048[] = {
